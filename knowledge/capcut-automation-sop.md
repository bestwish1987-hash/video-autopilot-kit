...
| 用戶說 | 動作 |
|---|---|
| 「跑 CapCut」「agent 剪片」 | Load `references/agent-brief-template.md` 拉模板 |
| 「Export v[N]」「匯出」 | Load `references/export-only-flow.md` 走 5 分鐘最短 Export path |
| 「字醜」「換字體」「綜藝風不對」 | Load `references/text-templates-catalog.md` 換模板策略 |
| 「Pro paywall」「卡關」 | Load `references/pro-paywall-map.md` 避雷地圖 |
| 「撞 daily 限制」 | Load `references/daily-limits.md` 重置時間 + fallback |
| 「不要 agent / 直接改」 | Load `references/json-direct-edit.md` skip GUI 走 JSON |
| 「CapCut 智慧助理 / 智慧編輯助理 / 內建 AI」 | Load `references/smart-edit-assistant.md`（實測：Beta 對話式 agent，慢且未驗證，captions 仍走確定性字幕面板）|
...
Reference → `references/json-direct-edit.md`
...
讀知識檔：capcut-agent-ops references/ 依任務 load 對應 path 文件
```
...
| Agent run | Tool calls | Duration | Tokens |
|---|---|---|---|
| Agent A (M32 first) — 整支剪片 from scratch | 333 | 68 min | ~高 |
| Agent B (M40 hit limit) — 套模板 partial | 146 | 28 min | ~2.9k |
| Agent C (continuation) — 套剩餘段 + Export | 145 | 23 min | ~92k |
| Agent D (Export only) — 純 Export | 37 | 4.7 min | ~40k |

→ **Path A (Export only) 是 most efficient**。能 JSON edit 完不要 GUI 操作。
...
- 「Export」「匯出 v[N]」 → load + references/export-only-flow.md
- 「字醜 / 綜藝風 / 換字體」 → load + references/text-templates-catalog.md
- 「Pro / paywall / 卡關」 → load + references/pro-paywall-map.md

不要在 brief 內 inline 重寫所有規則。Spawn 的 agent 收到 brief 寫「讀 capcut-agent-ops 知識檔 references/...md」就好。
...
教訓細節 → `references/smart-edit-assistant.md`（ASS 字幕陷阱 + PowerShell 截窗）、`references/programmatic-video-build.md`（端到端 ffmpeg pipeline）。canon M91-M96 → autopilot 知識庫的 meta-lessons-canon。
