...
**Hard cap：spawn 上限 = 2 agents / task**。連 2 個失敗 → 停止 spawn，改 Path D (JSON edit) 或 user manual。詳 `token-efficiency-lessons.md`。
...
```
- v18: 「冰淇淋旁加小動態字給我用 CapCut 不要再給我用 ffmpeg!!!!!!」→ 走 CapCut sticker 不 ffmpeg drawtext
...
| 用途 | 範例檔 |
|---|---|
| Export only（JSON 已 patch） | `examples/brief-export-only.md` |
| 套單一 template + Export | （待補：v0.2 加） |
| 多模板 + 貼圖 + Export | （待補：v0.2 加，⚠ 易撞 daily limit）|
| Template browse（用戶決策前 explore） | `examples/brief-template-browse.md` |
...
- Path A-E spec：`../SKILL.md`
- Token efficiency anti-patterns：`token-efficiency-lessons.md`
- Pro paywall 避雷地圖：`pro-paywall-map.md`
- daily-limit 重置時間 + fallback：`daily-limits.md`
- JSON direct edit Path D：`json-direct-edit.md`
- text template 套用 catalog：`text-templates-catalog.md`
